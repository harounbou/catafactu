...
from .transaction_management import fetch_df_from_db, get_db_connection, record_transaction
from .utils import find_image_path_for_color, get_full_image_path
from .pdf_generator import generate_order_pdf

# Helper function to generate unique keys
def generate_unique_key(base_key, identifier):
    return f"{base_key}_{identifier}"
...
def bon_de_commande_page(products_df):
    ...
    # Unique identifier for this page instance (e.g., timestamp)
    page_id = datetime.now().strftime("%H%M%S")
    ...
    with tab1:
        ...
        delivery_address = st.selectbox("Adresse de Livraison", known_addresses, key=generate_unique_key("bdc_address", page_id))
        if delivery_address == "Inconnue pour l'instant":
            delivery_address = st.text_input("Adresse personnalisée", key=generate_unique_key("bdc_custom_address", page_id))
        ...
        with col1:
            selected_product = st.selectbox(
                ...
                key=generate_unique_key("bdc_product_select", page_id)
            )
        with col2:
            qty = st.number_input("Quantité", min_value=1, value=1, key=generate_unique_key("bdc_qty", page_id))
        ...
        if selected_product != "Sélectionnez un produit...":
            ...
            color = st.selectbox("Couleur", ["Sans couleur"] + colors, key=generate_unique_key("bdc_color", page_id)) if colors else None
            ...
            if st.button("➕ Ajouter", key=generate_unique_key("bdc_add", page_id)):
                item = {
        ...
        if st.session_state['order_items']:
            ...
            if st.button("🗑️ Supprimer le dernier", key=generate_unique_key("bdc_remove", page_id)):
        ...
        selected_import = st.selectbox("Sélectionnez une importation", import_options, key=generate_unique_key("bdc_import_select", page_id))
        if selected_import != "Aucune":
            ...
            if st.button("✅ Confirmer l'importation", key=generate_unique_key("bdc_import_btn", page_id)):
                st.session_state['order_items'] = [
        ...
        shipping_method = st.selectbox("Méthode de Livraison", shipping_options, key=generate_unique_key("bdc_shipping", page_id))
        ...
        payment_option = st.selectbox("Option de Paiement", payment_options, key=generate_unique_key("bdc_payment", page_id))

        if st.button("✅ Créer Bon de Commande", type="primary", key=generate_unique_key("bdc_submit", page_id)):
                    ...
                    with col1:
                        try:
                            with open(pdf_path, "rb") as file:
                                st.download_button("📥 Télécharger", file, f"BonDeCommande-{order_id}.pdf", key=generate_unique_key("bdc_download", order_id))
                        except Exception as e:
                            st.error(f"Failed to open PDF: {e}")
                    if st.button("Réinitialiser le formulaire", key=generate_unique_key("reset", order_id)):
                        st.session_state['order_items'] = []
                        st.rerun()
                else:
        ...
        if not orders_df.empty:
            for i, order in orders_df.iterrows():
                with st.expander(f"Commande #{order['order_id']} - {order['order_date']} ({order['status']})"):
                    ...
                    new_status_display = st.selectbox("Statut", display_options, index=current_index, key=generate_unique_key("status", order['order_id']))
                    new_status = status_options[display_options.index(new_status_display)]
                    if st.button("Mettre à jour", key=generate_unique_key("update", order['order_id'])):
