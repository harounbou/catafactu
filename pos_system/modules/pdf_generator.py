...
def generate_receipt_pdf(
    ...
    watermark=""  # Added watermark parameter
):
    ...
    # Watermark (using the provided watermark parameter)
    if watermark:
        pdf.set_font("Arial", "B", 50 if "PAYÉ" in watermark else 40)
        pdf.set_text_color(0, 255, 0) if "PAYÉ" in watermark else pdf.set_text_color(255, 0, 0)
        pdf.rotate(45, x=105, y=150)
        pdf.set_xy(50 if "PAYÉ" in watermark else 30, 100)
        pdf.cell(0, 0, watermark if language == "French" else "مدفوع" if "PAYÉ" in watermark else "دفعة أولية", 0, 0, 'C')
    pdf.rotate(0)
    ...
    if payment_details:
        ...
        if payment_dict:
            ...
            for method, amount in payment_dict.items():
                if amount > 0:
                    method_ar = {"Espèces": "نقداً", "Virement": "تحويل بنكي", "CCP": "CCP", "Chèque": "شيك"}.get(method, method)
...
def generate_order_pdf(order_id, city, order_date, delivery_address, items, shipping_method, payment_option, created_by, manufacturing_notes="", watermark=None):
    """Generate a purchase order PDF for manufacturing orders with optional watermark."""
    output_dir = "generated_pdfs"
    os.makedirs(output_dir, exist_ok=True)
    ...
    for item in items:
        row_height = 20
        start_x = pdf.get_x()
        if item.get('Image'):  # Adjusted to match pos.py item structure
            full_img_path = get_full_image_path(item['Image'])
            if full_img_path and os.path.exists(full_img_path):
                img_width, img_height = calculate_image_dimensions(full_img_path, max_width_mm=22, max_height_mm=20)
                row_height = max(row_height, img_height + 2)
...
def generate_reservation_pdf(client_info, items, expiry_date, watermark, transaction_id, created_by):
    output_dir = "generated_pdfs"
    os.makedirs(output_dir, exist_ok=True)
    ...
    pdf.set_font("Arial", "B", 20)
    pdf.set_y(10)
    pdf.cell(0, 10, "Bon de Réservation - Takideco", 0, 1, 'C')
    pdf.set_font("Arial", size=10)
    pdf.cell(0, 6, f"N° Réservation: {transaction_id}  |  Date: {datetime.now().strftime('%d/%m/%Y')}  |  Émise par: {created_by}", 0, 1, 'C')
    pdf.cell(0, 6, f"Valide jusqu'au: {expiry_date.strftime('%d/%m/%Y')}", 0, 1, 'C')
    pdf.ln(10)
    ...
    for item in items:
        ...
        if item.get('Image') and os.path.exists(item['Image']):
            img_width, img_height = calculate_image_dimensions(item['Image'], max_width_mm=22, max_height_mm=20)
            row_height = max(row_height, img_height + 2)
            x = start_x + (col_widths[0] - img_width) / 2
            y = pdf.get_y() + (row_height - img_height) / 2
            pdf.image(item['Image'], x=x, y=y, w=img_width, h=img_height)
        else:
            pdf.cell(col_widths[0], row_height, "", border=1)
    ...
    pdf.set_y(-40)
    pdf.set_font("Arial", "I", 9)
    pdf.multi_cell(0, 5, "Merci de confirmer avant la date d'expiration.\nContactez-nous à contact@takideco.dz ou au 0542918226.", align='C')

    pdf_filename = f"BonDeReservation-{transaction_id}-{datetime.now().strftime('%d%m%Y')}.pdf"
    pdf_path = os.path.join(output_dir, pdf_filename)
    pdf.output(pdf_path)
    return pdf_path